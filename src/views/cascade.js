/* ============================================================
   CASCADE — the view. Ore to Action, actually quantified.
   ============================================================ */
...
let D = null;
const picks = { machine: 0, duty: 2, policy: 1, wage: 0 };
let openSource = null;
  ...
  return `
    <div class="cas__row" data-step="${step.id}">
      ...
      <div class="cas__body">
        ...
        <div class="cas__val">
          <b>${fmt(v, unit)}</b>
          <span class="cas__lbl">${step.name}</span>
        </div>
        <div class="cas__meta">
          <span class="cas__sub">${step.note}</span>
...
function branchCard(b, res) {
  ...
  return `
    <div class="cas__br">
      ...
      <div class="cas__brl">${b.name}</div>
      <div class="cas__brs">${b.note}</div>
      ${f ? `<div class="cas__brop"><em>${f.op} ${num(f.n)}</em> ${f.unit}</div>` : ""}
      <div class="cas__brf">
...
function render() {
  ...
  document.getElementById("casLead").innerHTML =
    `<span>${fmt(m.electrical_energy, "Wh")}</span> of electricity ·
     <span>${fmt(m.embodied_hardware, "g")}</span> of robot hardware ·
     <span>${fmt(m.refined_materials, "g")}</span> of engineering alloys ·
     <span>${fmt(m.lithosphere_ore, "g")}</span> of mined rock`;
...
function controls() {
  document.getElementById("casCtl").innerHTML = D.assumptions.map(a => `
    <div class="cas__as">
      ...
      <div class="cas__seg" data-as="${a.id}">
        ${a.options.map((o, i) =>
          `<button data-i="${i}" aria-pressed="${i === a.default}" title="${o.label}">${o.label}</button>`).join("")}
      </div>
  ...
  document.querySelectorAll("#casCtl .cas__seg").forEach(seg =>
    seg.addEventListener("click", e => {
      ...
      if (!b) return;
      const asId = seg.dataset.as;
      picks[asId] = +b.dataset.i;
...
export async function initCascade() {
  ...
  D.assumptions.forEach(a => { picks[a.id] = a.default; });

  const upd = document.getElementById("casUpd");
  if (upd) upd.textContent = D.meta.updated;
  controls();
  render();
}
