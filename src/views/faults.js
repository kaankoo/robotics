...
function detail(f) {
  ...
  document.querySelectorAll("#fltPanel [data-lag]").forEach(b =>
    b.addEventListener("click", () => { app.show("tml"); }));
...
function claim() {
  ...
  document.getElementById("fltClaim").innerHTML = `
    <button id="fltClaimBtn">
      ...
      The one that takes <b>${s.slowest.lead}</b> years reaches <b>${s.slowest.n}</b> stations.
    </button>`;
...
export async function initFaults() {
  ...
  claim();
  size();
  detail(faults.find(f => f.id === "strainwave-disruption") || faults[0]);
}
